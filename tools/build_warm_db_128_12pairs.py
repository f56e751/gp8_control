...
# 이 빌더는 기존 DB 를 건드리지 않는 **추가 DB** 를 만든다 (2026-07-28 사용자
# "초기값 128개로 db 추가로"). warm_db.pkl / warm_db_rt10_12pairs.pkl 은 그대로 둔다.
DB_PATH = SKILLS_DIR / "warm_db_128_12pairs.pkl"
...
# 2026-07-28 사용자 "12*12쌍 전부": 교차곱(144)으로 전환. False = TARGETS × P_STARTS
# 전 조합. 이미 빌드된 대각선 12쌍은 append 모드가 유지하므로 132조합만 새로 푼다.
# 교차곱이 다 차면 --shuffle-points 를 다시 써도 전 조합 exact-match 가 된다.
PAIRED = False
TARGETS: list[tuple[float, float, float]] = [
    # run_static_pick_throw.sh 의 TARGETS 배열 그대로 (2026-07-28 사용자 지정).
    # 이전 12pairs 빌더와 다름: x = 1.20/1.45/1.70 (1.441/1.683 아님),
    # z 는 전 행 0.05 고정 (거리별 0.020/0.085/0.150 아님).
    (1.20, 0.225, 0.05), (1.20, 0.075, 0.05),
    (1.20, -0.075, 0.05), (1.20, -0.225, 0.05),
    (1.45, 0.225, 0.05), (1.45, 0.075, 0.05),
    (1.45, -0.075, 0.05), (1.45, -0.225, 0.05),
    (1.70, 0.225, 0.05), (1.70, 0.075, 0.05),
    (1.70, -0.075, 0.05), (1.70, -0.225, 0.05),
]
...
MAX_TCP_Z = float(os.environ.get("GP8_MAX_TCP_Z", "0.85"))
...
# 128 = 2026-07-28 사용자 지정 ("초기값 128개"). 486점 격자에서 시드 고정 샘플
# 127개 + None. 총시간 ≈ 조합수 × 변형수 × (solve 1건 시간) ÷ 워커 — 변형 수가
# 유일한 실질 레버다 (워커는 이미 최대, expand:True로 이미 2× 가속, GPU 백엔드 없음).
# 조합당 128개를 모두 풀고 그중 게이트 통과 min-J 해 하나만 저장하므로, 넓힐수록
# 해 품질은 오르고 시간은 선형으로 는다. 이력: 4 -> 9 -> 24 -> 12 -> 54 -> 128.
...
def main() -> None:
    ...
    ap.add_argument("--json-log",
                    default=str(REPO / "tools" / "warm_db_build_log_128_12pairs.json"))
    args = ap.parse_args()
