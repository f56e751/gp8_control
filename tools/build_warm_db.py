...
TARGETS: list[tuple[float, float, float]] = [
    ...
    # tests/run_static_pick_throw.sh 의 현재 TARGETS 12개 (2026-07-23 사용자 지정).
    # x가 커질수록 z도 오르는 계단형 배치 — 각 행이 한 높이의 bin 열이다.
    # 이 좌표들이 DB에 있어야 static test가 cold가 아니라 warm polish로 돈다.
    # entry 조회는 XY 최근접이라 '실제 사용하는 z'로 직접 만들어 둬야 그 z에서
    # Cartesian 엔벨로프 안전성이 빌더 게이트로 검증된다.
    (1.000, 0.225, 0.040), (1.000, 0.075, 0.040),
    (1.000, -0.075, 0.040), (1.000, -0.225, 0.040),
    (1.241, 0.225, 0.105), (1.241, 0.075, 0.105),
    (1.241, -0.075, 0.105), (1.241, -0.225, 0.105),
    (1.483, 0.225, 0.170), (1.483, 0.075, 0.170),
    (1.483, -0.075, 0.170), (1.483, -0.225, 0.170),
]
...
INIT_VARIANTS = _make_init_variants(54)
