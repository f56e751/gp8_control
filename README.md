...
- **석션/던지기** — suction ON 은 진공을 "무장"만 하고, 실제 흡착은 실기처럼
  **접촉**에서 일어난다: 컵 면이 박스 윗면 위(footprint + 1 cm)에서 8 mm 이내로
  닿으면 박스가 **눌리기 시작** — 두께가 컵 면을 따라 줄고(최소 40 %,
  `GP8_SIM_CRUSH_MIN`) 바닥은 벨트에 붙어 있으며, 컵은 그 안으로 더
  들어갈 수 있다(벨로우즈). 컵이 하강을 멈추는 순간 그 찌그러진 자세로 MJCF
  weld 가 걸린다(활성화 시점 상대자세를 `eq_data` 에 기입; 박스 중심이 컵 면
  근처에 온다). OFF 는 weld 를 푼다. (예전엔 반경 10 cm 안 최근접 박스를
  하강 시작 시점에 공중에서 weld 해서, 컵이 0.03 m 까지 내려가는 동안 박스가
  벨트 아래로 끌려 들어갔고 컵 아래 56 mm 에 매달린 채 던져져 사거리가
  0.3 m 쯤 길었다.)
