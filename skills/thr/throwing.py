...
# tool=0.240: 2026-08-03 사용자 지시로 **URDF 기준(24 cm)으로 통일**.
# URDF(assets/yaskawa/yaskawa.urdf) 실측: link6 원점 → gripper 프레임 8 cm(d6),
# gripper 프레임 → TCP 프레임 24 cm (joint_TCP parentFramePos=[0.24,0,-0.05])
# ⇒ link6 원점 → TCP 32 cm. 종전 0.220(실물 실측 22 cm, 2026-07-16)은 URDF보다
# 2 cm 짧아 플래너 조준점과 시뮬 지오메트리가 어긋나 있었다.
# 주의: 이 값이 바뀌면 nlp_planner의 warm DB가 무효화된다(_warm_db_params의 dims)
# — 첫 실행이 cold multistart로 느려지고, 필요하면 --build-warm-db로 재구축한다.
GP8_DIMS = dict(d1=0.330, a1=0.040, a2=0.345, a3=0.040, d4=0.340, d6=0.080, tool=0.240)

# GP8 위치 한계 — **시뮬 URDF(yaskawa.urdf)의 joint limit 그대로**, planner 부호 컨벤션.
# pybullet getJointInfo로 읽은 실측값 (2026-07-23 대조 확인).
# 주의: planner는 U/R/B/T 부호가 URDF와 반대 (sim SIGN=[1,1,-1,-1,-1,-1]) —
# 비대칭 범위는 뒤집어 넣어야 한다.
#   URDF                → planner
#   J1 [-170,   +170]   → [-170,   +170]   (부호 동일)
#   J2 [ -65,   +145]   → [ -65,   +145]   (부호 동일)
#   J3 [ -70,   +190]   → [-190,    +70]
#   J4 [-190,   +190]   → [-190,   +190]
#   J5 [-135,   +135]   → [-135,   +135]
#   J6 [-360.6, +360.6] → [-360.6, +360.6]
# 실기(실물 로봇) 한계는 이 값과 별개로 throw_nlp에서 '교집합'으로 추가 적용된다
# (GP8_Q_MIN_REAL/MAX_REAL — J5·J6가 실기 쪽이 더 좁음).
GP8_Q_MIN = np.deg2rad([-170.0, -65.0, -190.0, -190.0, -135.0, -360.6])
